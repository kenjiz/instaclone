...
import 'package:supabase_flutter/supabase_flutter.dart' as supabase hide AuthUser;
import 'package:token_storage/token_storage.dart';

/// {@template supabase_authentication_client}
/// A Supabase implementation of the [AuthClient] interface.
/// {@endtemplate}
class SupabaseAuthenticationClient implements AuthClient {
  ...
  ///
  /// Emits [supabase.AuthUser.anonymous] if the user is not authenticated.
  @override
  Stream<AuthUser> get user {
    return _powerSyncRepository.authStateChanges().map((state) {
      final supabaseUser = state.session?.user;
      return supabaseUser == null ? AuthUser.anonymous : supabaseUser.toUser;
    });
  }
  ...
  Future<void> signUpWithPassword({
    ...
    try {
      await _powerSyncRepository.supabase.auth.signUp(
        ...
        password: password,
        data: data,
        emailRedirectTo: kIsWeb ? null : 'io._powerSyncRepository.supabase.flutterquickstart://login-callback/',
      );
    } catch (error, stackTrace) {
