      ...
      builder: (context, isThrottled, _) => widget.buttonBuilder(
        ...
        isThrottled || widget.onTap == null
            ...
            : () => _throttler.run(() {
                  ...
                  Future<void>.delayed(
                    Duration(
                      milliseconds: widget.throttleDuration ?? _throttler.milliseconds ?? 350,
                    ),
...
class _FadedButtonState extends State<FadedButton> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _opacityAnimation = _animationController.drive(CurveTween(curve: Curves.decelerate)).drive(_scaleTween);
    _setTween();
  }
...
class _ScaledButtonState extends State<ScaledButton> with SingleTickerProviderStateMixin {
  static final kScaleOutDuration = 150.ms;
  static final kScaleInDuration = 230.ms;
  final Tween<double> _scaleTween = Tween<double>(begin: 1);
  ...
  void initState() {
    ...
    _scaleAnimation = _animationController.drive(CurveTween(curve: Curves.decelerate)).drive(_scaleTween);
    _setTween();
  }
