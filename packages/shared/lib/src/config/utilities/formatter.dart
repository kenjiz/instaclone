  ...
  Formatters({
    ...
    DateFormat Function(String locale)? dateFormat,
        ...
        currency = NumberFormat.currency(
          symbol: Localizations.localeOf(context).languageCode == 'en' ? r'$' : '₸',
          decimalDigits: 0,
...
extension FormatString on String {
  ...
  String currencyFormat({
    required BuildContext context,
      ...
      _formatters(context).currency.format(formatToNum(context: context, separate: false));
...
extension FormatNum on num {
  ...
  /// Returns [String] with currency symbol.
  String currencyFormat(BuildContext context) => _formatters(context).currency.format(this);
}
...
extension FormatNullable on num? {
  ...
  /// Returns [String] with currency symbol.
  String currencyFormat({required BuildContext context}) => _formatters(context).currency.format(this);
}
...
extension CompactFormatter on num {
  ...
  String compact(BuildContext context) {
    ...
    final compactFormatter = NumberFormat.compactLong(locale: locale.languageCode);
    return this <= 9999
  ...
  String compactShort(BuildContext context) {
    ...
    return this <= 9999 ? format(context: context) : compactFormatter.format(this).replaceAll(',', '.');
  }
}
